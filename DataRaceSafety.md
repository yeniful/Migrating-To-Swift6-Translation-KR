...
이전에는 섬세한 런타임 동기화(Runtime Synchronization)를 통해 변경될 수 있는 상태(mutable state)를 수동으로 보호해야 했습니다. 프로그래머들은 `locks`, `queues`와 같은 도구들을 사용해서 데이터 경쟁을 막았습니다. 이와 같은 방식은 올바르게 하기도 어렵고, 시간이 지남에 따라 바르게 유지보수하는 것 또한 악명높을 정도로 어렵습니다. 또한 동기화가 필요한지 결정하는 것조차 힘들 수 있습니다. 최악의 경우, 안전하지 않은 코드가 런타임에서 실패를 일으킨다는 보장이 없습니다. 부정확하고 예측할 수 없는 데이터 경쟁의 특성상, 매우 특이한 조건에서 문제가 발생할 수 있기 때문에 이러한 코드는 잘 동작하는 것처럼 보입니다.
...
데이터 격리는 공유되는 변경 가능한 상태(Shared mutable state)를 보호하기 위해 사용하는 메커니즘입니다. 격리마다 독립적인 격리 단위가 존재하는데, 이를 격리 영역 (isolation domain) 이라고 합니다. 각각의 격리 영역이 보호해야 하는 상태의 범위는 매우 다양합니다. 어떤 격리 영역은 단일 변수만 보호할 수도 있고, 사용자 인터페이스 같은 전체 하위 시스템을 보호할 수도 있습니다.

격리 영역의 중요한 특징은 격리 영역이 만들어주는 안전성입니다. 변경 가능한 상태는 한 번에 하나의 격리 영역에서만 접근할 수 있습니다. 변경 가능한 상태를 한 영역에서 다른 영역으로 전달하는 것은 가능하지만, 여러 영역이 동시에 해당 상태로 접근하는 것은 불가능합니다. 컴파일러는 이와 같은 동시 접근을 방지합니다.
...
### 비격리 (Non-isolated)
함수와 변수가 반드시 격리 영역에 속해야하는 것은 아닙니다. 격리되지 않은 상태인 `non-isolated`가 기본값입니다. 데이터 격리 규칙에 따라 격리되지 않은 코드는 다른 영역에서 보호되는 상태에 접근해서 상태를 변경할 수 없습니다.

```
func sailTheSea() {
}
```
이 최상위 함수는 정적 격리가 없으므로 `non-isolated` 상태입니다. 따라서 다른 `non-isolated` 함수를 호출하거나 `non-isolated` 변수에는 안전하게 접근할 수 있지만, 다른 격리 영역에 있는 것들에 접근할 수 없습니다.
```
class Chicken {
    let name: String
...
```
이 클래스는 `non-isolated` 타입의 예시입니다. 상속은 정적 격리에 영향을 줄 수 있습니다. 그러나 이와 같은 슈퍼클래스나 채택한 프로토콜이 없는 단순한 클래스도 기본 격리(Default Isolation)를 사용합니다.
데이터 격리 덕분에 non-isolated 엔티티는 다른 격리 영역의 변경 가능한 상태에 접근할 수 없습니다. 이에 대한 결과로 `non-isolated` 함수와 변수는 언제나 어떤 다른 영역에서든 안전하게 접근할 수 있습니다.
